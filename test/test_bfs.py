...
def test_bfs_traversal():
    ...
    # Check if error is raised if starting node is missing
    with pytest.raises(ValueError, match= r"Starting node is not present in graph"):
        test.bfs('Jake Oxendine')

    empty=Graph("data/empty_network.adjlist")
    # Check if error is raised if empty test is used
    with pytest.raises(ValueError, match= r"Starting node is not present in graph"):
        test.bfs('Jake Oxendine')
    pass
...
def test_bfs():
    ...
    # Test correct implimentation of search
    ...
    # Test not connecting nodes return none
